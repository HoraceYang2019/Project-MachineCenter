...
# ====================== 主要功能：計算剩餘壽命 ======================
def calculate_remaining_life(csv_path, max_wear=0.3, pred_column='Final_Pred_i+2'):
    """
...
def plot_remaining_life(df, result_dict, output_path, max_wear=0.3):
    ...
    plt.plot(df['Cycle'], df['Final_Pred_i+2'], 'v--', color='red', linewidth=1.5, label='Model Prediction (i+2)')
...
# ====================== 主程式 ======================
def main():
    CONFIG = initialize_config('paths_B')  # 可改成 paths_B 或 paths
    
    csv_path = os.path.join(CONFIG['paths_B']['result_dir'], 'prediction_NKUST-B.csv')
    ...
    # 讀取原始資料畫圖
    df = pd.read_csv(csv_path)
    plot_path = os.path.join(CONFIG['paths_B']['visualization_dir'], 'RUL_estimation.png')
