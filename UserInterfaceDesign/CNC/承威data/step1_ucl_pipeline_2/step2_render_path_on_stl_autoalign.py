...
def main(
    ...
    output_dir: Path = None  # 新增此參數接收外部動態路徑
):
    # 1. 決定輸出目錄 (動態替換)
    ...
    STL_SCALE = 1.0  # 如果需要縮放 STL 模型，調整此值。通常不需要。
    ...
    # ========================================================
    # 注意：以下的程式碼必須與上面的 if/else 同層級，絕對不能縮排進去
    # ========================================================
    ...
    # 將原本的 OUTPUT_DIR 換成 target_dir
    out1 = target_dir / "path_on_stl_torque_value.html"
    ...
    out2 = target_dir / "path_on_stl_torque_ucl.html"
    ...
    out3 = target_dir / "path_on_stl_bending_value.html"
    ...
    fig4 = build_figure(
        ...
        title="CNC Path on STL - BendingX UCL (±)",
    )
    out4 = target_dir / "path_on_stl_bending_ucl.html"
    fig4.write_html(str(out4), include_plotlyjs="cdn", auto_open=False)

    print(f"Saved HTMLs to: {target_dir}")
